...
export async function updateProblem(problemId: number, input: ProblemUpdateInput): Promise<Problem> {
    const db = await getDb();
    const now = new Date().toISOString();
...
export async function markProblemSolved(problemId: number, input: ProblemSolveInput): Promise<void> {
    const db = await getDb();
    const now = new Date();
    ...
    // TODO: update 
    await db.execute(
        "UPDATE problems SET solve_count = ? WHERE id = ?",
        [newSolveCount, problemId]
    );
}
